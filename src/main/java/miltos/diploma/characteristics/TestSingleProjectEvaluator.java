...
public class TestSingleProjectEvaluator {
	...
	public static void main(String[] args) throws CloneNotSupportedException, InterruptedException{
		...
		/*
		 * Step 1.5: export resources needed for analysis to temporary file
		 */
		File projectRoot = new File(FileSystems.getDefault().getPath(".").toAbsolutePath().toString());
		File tools = new File(projectRoot.toString() + "/tools");
		File pmdDirectory = new File(tools.toString() + "/pmd-bin-5.4.1");
		File rulesetsDirectory = new File(projectRoot.toString() + "/Rulesets");

		tools.mkdirs();
		pmdDirectory.mkdirs();
		rulesetsDirectory.mkdirs();

		File pmdResource = new File("./src/main/resources/pmd-bin-5.4.1");
		File rulesetsResource = new File("./src/main/resources/Rulesets");
		File ckjmResource = new File("./src/main/resources/runable-ckjm_ext-2.2.jar");

		try {
			FileUtils.copyDirectory(pmdResource, pmdDirectory);
			FileUtils.copyDirectory(rulesetsResource, rulesetsDirectory);
			FileUtils.copyFileToDirectory(ckjmResource, tools);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void getConfig(){
		try {	
			...
			Document doc = builder.build(new File(System.getProperty("user.dir") + "/src/main/resources/config.xml").getAbsoluteFile());
